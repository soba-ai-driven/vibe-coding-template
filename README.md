...
画面（UI）の細かい指定は **言葉より画像のほうが速くて正確**です。

1. PowerPoint や Google Slides で画面のラフを描く（普段の要件定義書と同じ要領で OK）
2. そのスライドを**スクリーンショットして Claude のチャットに貼り付ける**
3. Claude が実装したら、実画面のスクショを返してくれる
4. 「ボタンをもう少し下に」「色を青に」など、自然な日本語で修正点を伝える
5. OK が出るまで 3〜5 往復繰り返す

> 💡 既存サービスの画面を参考にしたい場合も、そのスクショを送って「こんな雰囲気で作って」が有効です。
> 完璧な完成イメージである必要はありません。「ここにタイトル、その下にリスト、右上にボタン」程度の配置が分かれば十分です。

### 本番公開するとき

ステージングで動作確認したら、Claude に **「本番に反映して」「公開して」「リリースして」** のいずれかを伝えてください。Claude が本番反映までやってくれます。

> ⚠️ 「OK」「いいね」「動いた」だけだとお試し公開のままで止まります。本番に出すときは明示的に「公開して」と伝えてください。
