...
非エンジニアにとって、画面（UI）の細かい要件を**言葉で**伝えるのは難しいものです。「ボタンは右上で、リストは2列で、色は青系で…」と言葉で説明していると Claude も画面を想像しきれず、ずれていきます。

**画面については、画像でやり取りするのが一番速くて正確です。**

#### 推奨フロー

** 1. PowerPoint やスライドで画面イメージを描く**

普段の要件定義・設計書と同じ要領で、PowerPoint・Google Slides・Keynote 等で画面のラフを描きます。

> 💡 完成品である必要はありません。図形と文字で「ここにタイトル、その下にリスト、右上にボタン」程度の配置が伝われば十分です。

** 2. そのスライドをスクリーンショットに撮って Claude のチャットに貼り付ける**

スライドを画像保存（または画面をスクショ）→ Claude Desktop のチャット欄にドラッグ＆ドロップ、または貼り付け。
...
** 3. Claude が実装したらスクショで結果を見せてくれる**

Claude が画面を実装し、Playwright で実際の画面のスクリーンショットを撮って「こうなりました」と返してきます。

** 4. 差分を口頭でフィードバックする**

「もう少し余白を空けて」「ボタンを下にして」「色を緑にして」など、自然な日本語で修正点を伝えます。

** 5. OK が出るまで 3〜5 往復繰り返す**

1回で完璧を目指さず、画像で会話を重ねて磨き上げていくのがコツです。

> 💡 既存サービスの画面を参考にしたい場合も、そのスクショを送って「こんな雰囲気で」と伝えるのが有効です。
