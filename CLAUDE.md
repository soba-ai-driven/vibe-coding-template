...
| 操作 | いつ実行するか |
|---|---|
| `git push origin main` | コード変更があったら毎回 Claude が実行（Staging公開） |
...
**PR・マージ・push 等の技術用語をユーザーに使わせない。** ユーザーには「ステージング」「本番」の2語のみ使い、操作は全部 Claude が代行する。

- コード変更したら staging に自動デプロイし、「ステージング URL で確認してください: https://...」とだけ伝える
...
「PR を作ってください」「マージしてください」「push してください」のような依頼は、非エンジニアのユーザには理解が難しいので**絶対にしない**。
...
### 4.3 UI 要件のヒアリングと反映フロー（最重要）

非エンジニアは UI を**言葉で説明するのが苦手**。PowerPoint 等で画面案を書いた方が早い。だから「言葉で詳細に書いて」と要求せず、**画像でやり取りする**前提でフローを組む。

#### 受け付け方

ユーザーが画面を新規作成・大きく変更したいと言ったら、まず:

```
画面のイメージはどこかにありますか？
PowerPoint やスライドに描いたものがあればスクリーンショットでチャットに貼ってください。
無ければ、参考にしたい既存サービスのスクリーンショットでも OK です。
（言葉で説明していただいてもいいですが、画像があった方が早く正確に作れます）
```

ユーザーが画像をアップロードしたら、それを「正」として実装する。

> 💡 言葉だけの要件（例: 「ユーザー一覧画面を作って」）でも作れるが、その場合は **Claude が最初に簡素なバージョンを作って → スクショ → ユーザーに「これでイメージ合ってますか？」と確認** のループに入る。

#### 反映フロー

1. **実装する**: 受け取った画像 or 言葉の要件から最初の版を実装する
2. **ローカル確認**: `npm run dev` でローカル起動 → Playwright MCP で該当画面のスクショを撮る
3. **比較を提示**: 「実装した画面はこちらです（スクショ）。元のイメージと比較してください」とスクショを返す
4. **差分指示を受ける**: ユーザーから「ここをこう変えて」「色を青に」等の自然言語フィードバックを受ける
5. **再修正→再スクショ→再提示** を、ユーザーが OK と言うまで繰り返す
6. **OK が出たらステージングに反映**

#### やってはいけないこと

- 言葉の要件だけで30分悩む → ダメ。早めに簡素版を作って画像で会話に切り替える
- 「もっと詳しく教えてください」と質問攻め → ダメ。画像で「これでどうですか」と提案する形にする
- 完璧主義 → ダメ。1回で完璧に仕上げず、3〜5往復のスクショ比較ループで磨く前提
